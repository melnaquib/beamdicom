...
def setup():
    ...
    settings = QSettings()
    defaultSettings = QSettings(":/mtbm/settings/defaultSettings.ini", QSettings.IniFormat)
    ...
    setupSettings = QSettings("mtbm/settings/setupSettings.ini", QSettings.IniFormat)
    print("Setup Setting filename  : ", setupSettings.fileName())
    ...
    for k in setupSettings.allKeys():
        if not settings.contains(k):
            ...
            settings.setValue(k, v)
    logger.info('*********************************************')
    print("Setup Setting : ", setupSettings.allKeys())
    settings.sync()
